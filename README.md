...
| `↑` `↓` | 分子⇄分母、極限の上⇄下、行列の行間移動。直前が裸の大型演算子(∑ や lim)ならバンドに昇格して極限に入る |
| `Enter` | 行列/グリッド内: 下に行を追加(`\addcol` `\delrow` `\delcol` も) |
| `Backspace` | 中身のある構造には踏み込み、空の構造は削除 |
| `Shift+←` `→` | ブロック選択(兄弟ノード範囲、背景色でハイライト)。選択中に `\cancel` `\frac` `\sqrt` `^` `_` `(` で包む、`Backspace` でまとめて削除 |
| `Shift+↑` | 選択を親構造へ拡大(今いるインセット全体 → さらに外側 → 数式全体) |
| `Ctrl+C` `Ctrl+X` `Ctrl+V` | 選択のコピー / カット / ペースト(カット+移動先でペースト=移動) |
| `Ctrl+G` | ジャンプモード(EasyMotion 風): 最大16個のラベルを重ね表示し、ラベルキーで移動。未入力スロット→行列/ベクトルの各セル(自分のいるグリッド優先)→遠端から距離半減の等比配置、の順で近距離〜文書両端をバランス良くカバー(`a` が最有力) |
...
| `Ctrl+B` | ブロック選択モード: 各構造ブロックをラベル+深さ別の背景色で表示し、ラベルキーでそのブロックを丸ごと選択(そのまま ^C/^X・包み込み・削除へ) |
| `Ctrl+O` | 構造ビュー: 全ブロックの入れ子構造を深さ別の背景色で一気に表示 |
| `Ctrl+Y` | 正準AAをクリップボードへコピー |
| `Ctrl+S` | LaTeX を保存 / `Ctrl+T` イタリック表示切替 |
| `Esc` | モード解除(選択→構造ビューの順)。解除するものが無ければ**終了**(`Ctrl+Q` も終了だが、端末に取られる環境がある) |
