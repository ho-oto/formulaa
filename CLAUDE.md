...
```sh
cargo test                      # all tests (unit + roundtrip)
...
| `src/editor/mod.rs` | the structure editor (LyX-style cursor; insert/move/delete/select; staged unwrap) |
| `src/editor/modes.rs` | ^F free cursor · ^B block select · ^T grid edit (`GridSel`) · display decoration (`decorated`) |
...
| `src/theme.rs` | TUI colors (bin only) |
| `src/main.rs` | main loop + CLI subcommands (`--debug` flag) |
| `src/tui.rs` | drawing: layout, scrolling, marker/selection painting, popup/preview overlay |
