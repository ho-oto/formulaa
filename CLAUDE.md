...
| ├ `atoms.rs` | **全記号語彙のテーブル3枚**(すべて phf・手書き): 出力側 `ATOMS`(char → LaTeX 綴り+`kind` Sym/BigOp)、入力側 `NAMES`(綴り → char、別綴り・ASCII 絵文字綴りは `\|` で併記。コマンド別綴りは `resolve` の match パターン)、否定 `NEGATIONS`/逆引き `UNNEGATIONS`(基底 char ⇄ 斜線付き char。`!` 前置/後置綴りと `\!` トグルの解決元、鏡映性はテーブル走査のテストが固定)・予約グリフ・`is_atom`。**入力できる原子は必ず LaTeX 綴りを持つ**(gap=0 をテストが固定) |
