...
- ceil/floor は `⌈ ⌉ ⌊ ⌋`(複数行はブラケット片を片角だけ:
  ...
  ノルムは 1行 `‖`・複数行 `║` の全高列(ノルム直下のノルム入れ子は
  書けない — 内側は `⎸ ⎹` で)。
...
U+2024(原子の `.` とは別字)、ddot は `․․`(右に1列はみ出し、その列の
基線は空白のまま)、underline は真下に `¯`、utilde は真下に `˜`
(チルダは AST と描画が上下で入れ替わる対。浮いた `¯ ^ ˜ ˇ ˚ ˙ ¨`/`‗`
も寛容に読める)
```
⇀       ⇀        ˰
...
### ローマン体 — 裸のランか `'…'`、テキストは `"…"`
`dx` `asiny` のような英字ラン(非辞書語)はそのまま \mathrm。
`i.i.d.` `w.r.t.` のような略語もドットごと1ランで \mathrm(内部の `.` は
