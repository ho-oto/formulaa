...
┌   ┬   ┐      ⎡   ┬   ⎤
  a   b        ⎢ a   b ⎥
├   ┼   ┤      ┠   ┼   ┨    ← \begin{bmatrix} 相当(融合形。枠付き格子を
  c   d        ⎢ c   d ⎥       括弧で挟んで書いても同じ意味に整形される)
└   ┴   ┘      ⎣   ┴   ⎦
```
...
```
`─` のランに先端が直接続けば矢印、空白を挟めば分数+矢印原子
(`─── →`)。二重矢印は `══⇒` / `⇐══`。

### ローマン体テキスト — `"…"`
`∫f(x)"dx"` → `\mathrm{dx}`。空白を含むときは `␣` で書く:
`"if␣x"` → `\text{if x}`。引用符の外の ASCII はイタリック原子のまま。
