...
- 左右不一致も可: `(0,1]`。
- **ヌルデリミタ** 左 `▏` / 右 `▕`: 「片側なし」や裸のグリッドの範囲
  マーカー(LaTeX の `\left.`/`\right.` 相当)。

### 行列・グリッド — `⎡ ⎤` の中に素のグリッド
列区切り=**空白2列**(グリッドが1行だけなら**4列**)、行区切り=**空白1行**。
基線=中央行。
```
⎡a  b⎤       𝑥 = ⎧𝑥   𝑥≥0▕
⎢    ⎥           ⎨       ▕     ← cases は { と ヌル右 ▕ の組
⎣c  d⎦           ⎩−𝑥  𝑥<0▕
```
括弧なしのグリッドは `▏a    b▕`(1行なので区切り4列)のように
ヌルデリミタで範囲を示す。
...
### 明示空白 — `␣` (U+2423)
見た目のための空白は実スペースでなく可視の `␣` を置く(実スペースの列は
「区切り」として消費される)。LaTeX `\ ` / Typst `space` に変換される。
...
`─ ┄ │ √∛∜ ⎛⎜⎝⎞⎟⎠ ⎡⎢⎣⎤⎥⎦ { } ⟨ ⟩ ⎧⎪⎨⎩⎫⎬⎭ ╱ ╲ ⎸ ⎹ ▏ ▕ ⬚ ▌ ▶
^ ˜ ¯ ⇀ ˙ ¨ ˇ ˘ ˚ ‗`、基線より上の行の `_`、数学イタリック体(𝐴–𝑧, ℎ)、
インライン上付き・下付き文字。これら以外の Unicode 数式記号(α ≤ ∈ → ℝ ⊗ …)は自由に使える。
...
- 積分・総和の極限は必ずバンド `┄` で。`∑` の上下にただ置くだけでは
  入れ子(極限の中の ∫ など)が曖昧になる。手書きでは `~~∑~~` のように
  `~` でもよい(`mascii fmt` が `┄` に直す)。
