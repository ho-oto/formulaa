...
| `\` | ミニバッファを開く(下の「コマンド」参照。`Enter`/`Space` 確定、`Esc` 取消) |
| `"` | 予約(テキストは `\text…` / `\rm…` で) |
...
| `Shift+↑` | 選択を親ブロックへ拡大 |
| `Ctrl+B` | **ブロック選択モード**: 構造ブロックにラベルが付き、キー1つでそのブロック全体を選択 |
...
グリッド上を自由に動くカーソルと、最寄りの編集位置へのスナップ先が
同時に表示されます。
...
| `d` / `D` | 現在の行 / 列を削除 |
| `Enter` | 行を下に追加(通常時と同じ) |
| `Esc` / `Tab` / `Ctrl+E` | モード解除(カーソルが行列外に出ても自動解除) |
...
| `\op` | **名前ボックス**を開く: 英数+スペースで名前を入力し `Enter`/`Tab` で確定 → 立体ラン(=\mathrm、辞書語は \sin 等の関数に)。スペース区切りは ␣ で連結 |
| `\op*` | 同上、確定で ┄バンド┄ になり下極限へ(スペース区切りは `┄ess┄sup┄` のような複数 piece に)。`Esc` 取消、名前以外のキーは確定してから通常動作 |
