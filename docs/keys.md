...
| `Esc` | モード解除(フリー/グリッド/ミニバッファ/ボックス)→ 選択解除 → 終了(`Ctrl+Q` は端末に取られることがあるので Esc が確実) |
...
ある関係すべて(正式名は `\ne` `\notin` `\nleq` `\nsubset` …)。
また `\!` 単独は**直前の記号を否定形に置換**するコマンドです
(`=` の直後に `\!` → ≠。否定形を持たない記号ではエラー)。
打ち消し線(\cancel)そのものは非対応です。

## モードの重なり(内部構造)

キーは次の順で解釈されます: フリーカーソル → ミニバッファ →
\op ボックス → (Ctrl コード → グリッド編集 → 通常キー)。
