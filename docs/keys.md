...
| `\` | コマンド入力(下の「コマンド」参照)。入力中の `\command` は**カーソル位置に重ねて表示**され(未知の名前の間は赤)、既知の名前は**確定結果をその場にプレビュー**(記号はカーソル位置のゴースト文字、`\frac` などの構造は直下に ⬚ スロット付きの形)。プレビューも補完も数式の**上に浮かぶ**だけなので、下の数式はずれない。`Tab` で補完一覧(下記)。`Enter`/`Space` で確定、`Esc` 取消。`\`+`Space` は意味のある空白 ␣ |
...
| `Delete` / `Ctrl+D` | カーソルの右を削除(`Backspace` の逆向き) |
...
`\` に続けて途中まで打ってから `Tab` を押すと、カーソルの下に補完一覧が
出る。各行は**挿入されるシンボルと、その綴り**:
...
| キー | 動作 |
|---|---|
| `Tab` | 一覧を開く(開いていれば次の行へ) |
| `↑` `↓` | 行を選ぶ(端で折り返す) |
| `Enter` | 選んだ行を確定(その行の一番長い綴りが使われる) |
| 続けて入力 / `Backspace` | 一覧が絞り込まれる |
...
`\lr` の │ 区切りを持つ括弧と、行列を包む括弧(`\pmatrix` など)は
対象外 — 取り出すべき「中身」が一つに定まらないので、従来どおり
カーソルが括弧の外に出る。
