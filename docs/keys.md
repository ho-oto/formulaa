# mascii キーボードマニュアル

TUI エディタ(`cargo run` / `mascii`)の全キー・全コマンドのリファレンス。
エディタは LyX 型の構造エディタで、カーソルは常に「数式構造の中の編集位置」
にあります。同じキーマップが VSCode / Obsidian / Zed 拡張(wasm)でも
使われます。

## 基本入力

| キー | 動作 |
|---|---|
| 英数・記号 | 原子を挿入(英字はイタリック変数 𝑥 に。`-`→−、`*`→∗) |
| `^` / `_` | 上付き / 下付きに入る(可能なら `x²` `aᵢ` とインライン表示) |
| `(` `[` `{` | 自動サイズ括弧に入る(選択中はそれを包む) |
| `)` `]` `}` | 括弧を閉じて外へ(`]` は行列からの脱出も) |
| `//` | 分数(`/` 2連打。リテラル `//` は `/ /` と打って間の空白を消す) |
| `Space` | 整形用の空白(LaTeX に出ない。意味のある空白は `\space`→␣) |
| `\` | コマンド入力(下の「コマンド」参照)。入力中の `\command` は**カーソル位置に重ねて表示**され、`Enter`/`Space` で確定、`Esc` 取消 |
| `"` | 予約(テキストは `\text…` / `\rm…` で) |

## 移動

| キー | 動作 |
|---|---|
| `←` `→` | 構造の中を通り抜けながら移動(選択中は選択の左端/右端へ集約) |
| `↑` `↓` | 分子⇄分母、極限の上⇄下、行列の行移動。直前が裸の ∑/lim ならバンドに昇格して極限へ。縦ターゲットがなければ複数行数式の行間移動(列位置を保持) |
| `Tab` | 現在のインセット(分数・根号・極限・括弧など)から抜ける |
| `Home` / `End` | 現在行の先頭 / 末尾(複数行数式では Break 区切りの行単位) |
| `Ctrl+A` / `Ctrl+E` | 数式全体の先頭 / 末尾へ |
| マウスクリック | クリック位置に最も近い編集位置へ(端末のテキスト選択は Shift+ドラッグに) |
| `Ctrl+G` | **ジャンプモード**(下記) |
| `Ctrl+F` | **フリーカーソルモード**(下記) |

## 複数行数式

| キー | 動作 |
|---|---|
| `Enter`(トップレベル) | 数式の改行。行間に `┈` 1文字の区切り行が入る(align 相当の桁揃えはなし) |
| 行頭で `Backspace` | 前の行と結合 |
| `↑` `↓` | 行間移動(列位置をなるべく保持) |

## 選択・クリップボード

| キー | 動作 |
|---|---|
| `Shift+←` `Shift+→` | 選択を伸縮(背景色で表示) |
| `Shift+↑` | 選択を親ブロックへ拡大 |
| `Ctrl+B` | **ブロック選択モード**: カーソルの祖先チェーンをグラデーション背景+ラベルで表示。↑/→=外側へ、↓/←=内側へ、Enter/ラベルキーで選択、もう一度 `Ctrl+B`/Esc で解除 |
| `Ctrl+C` / `Ctrl+X` / `Ctrl+V` | コピー / カット / ペースト |
| `←` / `→`(選択中) | 選択を解除して左端 / 右端へ |
| `Backspace` / `Delete`(選択中) | 選択範囲を削除 |
| 構造キー(選択中) | `^` `_` `(` `[` `{` `\frac` `\sqrt` `\cancel` は選択を包む |

## undo / redo

| キー | 動作 |
|---|---|
| `Ctrl+Z` | undo(数式が変わった操作ごとに1ステップ。カーソルも当時の位置に戻る) |
| `Ctrl+R` | redo(新しい編集をすると redo 履歴はクリア) |

## ジャンプモード(`Ctrl+G`)

EasyMotion 風。編集位置の候補にラベル(`a s d f …`)が重なって表示されます。

| キー | 動作 |
|---|---|
| ラベル文字 | その位置へジャンプ |
| `←→↑↓` | マーカーの選択を物理的な方向で移動(選択中は色が変わる) |
| `Enter` | 選択中のマーカーへジャンプ |
| その他のキー | キャンセル(展開されたスロット表示は次の入力まで維持され、レイアウトが揺れない) |

空きスロット(⬚)と親要素の端が優先され、遠距離は2段階ジャンプで到達
します。もう一度 `Ctrl+G` を押すと表示そのままで再ラベルされます。

## フリーカーソルモード(`Ctrl+F`)

グリッド上を自由に動くカーソル(専用色)と、最寄りの編集位置への
スナップ先が同時に表示されます。

| キー | 動作 |
|---|---|
| `←→↑↓` | セル単位で自由移動(未展開の添え字・スロットに近づくと自動展開) |
| `Ctrl+G` | ジャンプマーカーを表示。ラベル文字、または矢印で選んで `Enter` でそのマーカーへ移動し、**フリー移動に戻る**。`Esc`/もう一度 `Ctrl+G` でマーカーだけ消す |
| `Enter` | スナップ先の編集位置に確定してモード終了 |
| `Esc` など | キャンセル |

## グリッド編集モード(`Ctrl+O`、行列の中で)

行列・グリッドの「行を消したい」「列を足したい」を1キーで。

| キー | 動作 |
|---|---|
| `←→↑↓` | セル間を移動(端で停止) |
| `r` / `R` | 行を下 / 上に追加 |
| `c` / `C` | 列を右 / 左に追加 |
| `d` / `D` | 現在の行 / 列を削除 |
| `Enter` | 行を下に追加(通常時と同じ) |
| `Esc` / `Tab` / `Ctrl+O` | モード解除(カーソルが行列外に出ても自動解除) |

Ctrl 系(`Ctrl+Z` undo など)はモード中もそのまま使えます。

## 行列(通常時)

| キー / コマンド | 動作 |
|---|---|
| `Enter`(グリッド内) | 下に行を追加 |
| `]` | 行列から脱出 |
| `\addrow` `\addcol` `\delrow` `\delcol` | 行・列の追加/削除(グリッド編集モードのほうが速い) |

## 表示・出力・終了

| キー | 動作 |
|---|---|
| `Ctrl+T` | イタリック表示の切替(表示のみ。正準形は常にイタリック) |
| `Ctrl+Y` | 正準 AA をクリップボードへ |
| `Ctrl+S` | LaTeX を保存(起動引数のパス、既定 formula.tex) |
| `Esc` | 選択解除 → 構造ビュー解除 → 終了(`Ctrl+Q` は端末に取られることがあるので Esc が確実) |

## ミニバッファコマンド(`\` で開く)

### 構造

| コマンド | 動作 |
|---|---|
| `\frac` | 分数(選択中はそれが分子に) |
| `\sqrt` `\cbrt` `\qdrt` | √ ∛ ∜ |
| `\cancel` | 打ち消し(選択を包める) |
| `\matrix` | 2×2 行列。`\matrix34` で 3行×4列 |
| `\pmatrix` `\Bmatrix` `\vmatrix` `\cases` `\rcases` `\array` | 括弧違いの行列(同様に `\cases32` などサイズ指定可)。`\array` は裸の格子 |
| `\overbrace` `\underbrace` | ╭──╮ / ╰──╯(ラベル付き) |
| `\xto` `\xfrom`(`\xrightarrow` `\xleftarrow`)/ `\xTo` `\xFrom` | ラベル付き伸縮矢印 → ← / ⇒ ⇐ |

### デリミタ

| コマンド | 動作 |
|---|---|
| `\lr(]` `\lr{|}` `\lr\langle||\rangle` など | **見た目の順**のデリミタ指定(左・middle…・右)。`.` はヌル(片側なし、`┆`/`┊` で表示) |
| `\ceil` `\floor` `\norm` | ⌈ ⌉ / ⌊ ⌋ / ‖ ‖(ノルム直下のノルム入れ子は非対応) |
| `\mid` | 現在のデリミタ内に │ 区切りを追加(braket・集合の内包表記) |

### 演算子・関数

| コマンド | 動作 |
|---|---|
| `\sum` `\int` `\prod` … | 大型演算子の原子(↑/↓ でバンドに昇格して極限へ) |
| `\lim` `\max` `\min` `\sup` `\inf` `\det` `\gcd` `\Pr` `\plim` `\injlim` `\projlim` | ┈バンド┈ を作り下極限へ |
| `\argmax` `\argmin` `\limsup` `\liminf` | 複数語バンド(`┈arg┈max┈` `┈lim┈sup┈` — 語ごとに piece)。LaTeX/Typst にネイティブ名があれば(\limsup)それで出力 |
| `\op` | **名前ボックス**を開く: 英数で名前を入力し `Enter`/`Tab`/`Space` で確定 → 立体ラン(=\mathrm、辞書語は \sin 等の関数に) |
| `\op*`(別名 `\limits`) | 同上、確定で ┈バンド┈ になり下極限へ。**スペースは piece 区切り**(`ess sup` → `┈ess┈sup┈`)。`Esc` 取消、名前以外のキーは確定してから通常動作 |

### テキスト・空白・アクセント

| コマンド | 動作 |
|---|---|
| `\rm` | **名前ボックス**: 英数と `.` で入力し確定 → 立体ラン(`i.i.d.` 可、辞書語は関数に)。`\rm<chars>` の直結形も可 |
| `\text` | **ボックス**: 自由なテキストを入力し確定 → `"…"`(\text)。`\text<chars>` の直結形も可 |
| `\space` | 意味のある空白 ␣(LaTeX `\ `) |
| `\^z` `\z^` `\_10` `\^gamma` など | 上付き/下付きノードを挿入(`^`/`_` は名前の前後どちらでも可。1文字英数の連なりか記号名) |
| `\hat` `\vec` `\bar` `\dot` `\ddot` `\tilde` `\check` `\ring` `\underline` `\utilde` | 直前の1文字にアクセント(続けて実行で重ね掛け)。**選択があれば伸縮アクセント**(`┈┈˰┈┈` などのマーク入り範囲行が基底の上下に付く \widehat 形。分数など背の高い選択も可) |

### 記号

`\alpha` `\infty` `\to` `\in` `\RR` … 4000+ 記号(`->` `+-` `oo` のような
ASCII 名も)。上の名前と重ならなければすべて記号テーブル引きです。

## モードの重なり(内部構造)

キーは次の順で解釈されます: ジャンプ → ブロック選択 → フリーカーソル →
ミニバッファ → \op ボックス → (Ctrl コード → グリッド編集 → 通常キー)。
モード系のキー処理はすべて共有キーマップ(`src/input.rs`)にあり、
TUI と wasm で挙動が一致します。
