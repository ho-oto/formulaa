...
| `\` | コマンド入力(下の「コマンド」参照)。入力中の `\command` は**カーソル位置に重ねて表示**され(未知の名前の間は赤)、既知の名前は**確定結果をその場にプレビュー**(記号はカーソル位置のゴースト文字、`\frac` などの構造は直下に ⬚ スロット付きの形)。プレビューも補完も数式の**上に浮かぶ**だけなので、下の数式はずれない(両者は同じ箱の大小 — 同じ灰地・同じ位置に出て、`Tab` で場所は動かない)。`Tab` で補完一覧(下記)。`Enter`/`Space` で確定、`Esc` 取消。`\`+`Space` は意味のある空白 ␣ |
