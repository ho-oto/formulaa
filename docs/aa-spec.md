...
| `{ } ⎧⎪⎨⎩ ⎫⎪⎬⎭` | 波括弧(頂点 ⎨⎬ は基線行)。**高さ2は `⎰⎱` U+23B0/23B1**(左=⎰の下に⎱、右=⎱の下に⎰。上下どちらが ⎰ かで側が決まる) |
...
| `‖` U+2016 | ノルム: `‖` の全高列(複数行は同じ `‖` を縦に積む)。側は行内の出現パリティで決まるため、**ノルム直下のノルム入れ子は非対応** — 内側は `⎸ ⎹` か `\lr` で回避 |
...
| `^ ˜ ¯ ⇀ ˙ ¨ ˇ ˚` | 上アクセントマーク(原子の `~` `→` とは別コードポイント)。すべて**基底に密着する形で描画**: bar `¯`→`_`、hat `^`→`˰` U+02F0、tilde `˜`→`˷` U+02F7、check `ˇ`→`˯` U+02EF、ring `˚`→`˳` U+02F3、dot `˙`→`․` U+2024、vec `⇀`→`￫` U+FFEB(wide の `──>` と同じ普通の矢印の見た目)、ddot `¨`→`․․`(**右に1列はみ出す** — はみ出し列の基線は空白で、他に何も置けない)。AA に現れるのは密着形のみ(AST マーク文字を直接置く旧形式は廃止。breve `˘` も廃止) |
...
| `( )` | `( )` | `⎛⎜⎝ ⎞⎟⎠`(基線マークなし) |
| `[ ]` | `[ ]` | `⎡⎢⎣ ⎤⎥⎦`(同上) |
| `{ }` | `{ }` | `⎧⎪⎨⎩ ⎫⎪⎬⎭` — **頂点 ⎨/⎬ は常に基線行**(角より優先)。h=2 は `⎰⎱`(頂点なし — 基線は内側再帰) |
| `⟨ ⟩` | `⟨ ⟩` | 腕 `╱ ╲`、**頂点 ⟨/⟩ は常に基線行** |
| `\|` | `⎸ ⎹` | 全行 `⎸`(左)/`⎹`(右) |
...
- 基線: `⎨ ⟨` 系は頂点行から直接。`⎡ ⎛ ⎸ ┆`(と融合丸括弧の `╭` 列)は、
  内側に融合グリッドマーカーがあれば extent 中央行、なければ内側
  (最初の seg)を再帰。h=2 の `⎰⎱` も内側再帰。
