...
| 文字 | 用途 |
|---|---|
| `▌` | カーソル(1列を占有するため、カーソル付き描画は正準形とレイアウトもずれる) |
| U+E000–E0FF(私用領域) | ジャンプラベル(Ctrl+G)・ブロック強調(Ctrl+B)の表示マーカー |
...
寛容にしない部分: 基線上のトークンの真上・真下に(消費されたアクセント
マーク以外の)内容が重なっている入力は**エラー**(黙って捨てない)。
