...
### Cancel(打ち消し線)
- 打ち消しは**原子形トークン1個**(Sym / Func / Roman / Text)を包む
  `Cancel(CancellableNode)`(4形だけの専用 enum — 制約は型が持つ)。トークンの全ての非空白セルに結合ロングソリダス
  U+0338 を後置(`y̸`)。結合文字はセルを消費しないので列モデルを壊さない
  (パース時に前のセルのフラグへ折り込む。U+0336 も受理)。
- パース: トークンを読んだ場所でスパンのフラグを検査 — 非空白セルが全部
  打ち消されていれば `Cancel` に包み、一部だけならエラー(Text 内の実
  スペースには線を掛けるグリフがなく、フラグの有無を問わずトークンごと
  吸収する)。英字ランはフラグ境界でトークンが割れるため部分打ち消しに
  ならない(`si̸n` は `s`・`i̸`・`n` の3トークン)。構造グリフ(罫線・
  括弧・格子縁・スクリプト)上のフラグはどのトークンにも回収されず、
  パース終端の検査でエラー(黙って落とさない)。
- 正規化: 裸の原子規則は線越しにも適用(1文字 Func → Roman、非英字の
  Roman → Sym、空の Text/Func は消える)。構造への打ち消しの押し下げは
  normalize の仕事ではない — エディタと LaTeX リーダーが `cancel_all` で
  行う(型が迷子のラッパー自体を許さない)。**空の Sup/Sub
...
- Cancel の中の原子にも裸の原子規則を適用(1文字 Func → Roman、非英字
  Roman → Sym)。
