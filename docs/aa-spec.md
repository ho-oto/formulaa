...
| `‖` U+2016 / `║` U+2551 | ノルム: 1行は `‖`、複数行は `║` の全高列(罫線なので縦につながる)。側は行内の出現パリティで決まるため、**ノルム直下のノルム入れ子は非対応** — 内側は `⎸ ⎹` か `\lr` で回避 |
| `╎ ┆` U+254E/2506 | ヌルデリミタ(左/右、`\left.`/`\right.` 相当。破線=幽霊系で `┄` と揃える。旧 `▏ ▕` U+258F/2595 も入力として受理) |
| `┌┬┐ ├┼┤ └┴┘` | 裸グリッドの格子マーカー(区切りの交点。外周を含む) |
| `┠ ┨` | デリミタ融合グリッドの行区切りジャンクション |
| `⬚` U+2B1A | 「空」の綴り: 空スロット・行頭スクリプトの明示基底(LaTeX の空グループ `{}` に相当する形式の一部) |
| `␠` U+2420 | 整形スペーサ(`Node::Spacer`)の明示形。セッションファイルが再起動でスペーサを失わないための綴りで、パースは空白1列と同様に扱い正準出力では空白に戻る |
| `^ ˜ ¯ ⇀ ˙ ¨ ˇ ˚` | 上アクセントマーク(原子の `~` `→` とは別コードポイント)。すべて**基底に密着する形で描画**: bar `¯`→`_`、hat `^`→`˰` U+02F0、tilde `˜`→`˷` U+02F7、check `ˇ`→`˯` U+02EF、ring `˚`→`˳` U+02F3、dot `˙`→`․` U+2024、vec `⇀`→`￫` U+FFEB(wide の `──>` と同じ普通の矢印の見た目)、ddot `¨`→`․․`(**右に1列はみ出す** — はみ出し列の基線は空白で、他に何も置けない)。浮いた元グリフも寛容に読む(breve `˘` は廃止) |
...
- ddot(`¨`)は `․․` — 2つ目の `․` が**基底の右隣の列にはみ出す**。
  ...
  1文字 \mathrm は裸にならず引用付きのまま(glue 判定は辺ごと)。
  隣に √ があるとその貪欲な `_` オーバーラインと同じ行で融合しうるため、
  兄弟境界で `_` 同士が接触する場合は分離空白1を挟む(render の幾何規則)。
...
```
  ...
  旧形式の斜めペア `╱╲` / `╲╱` も寛容に読む)、
  vec は `─` 胴体+ASCII の `>` 頭(\xrightarrow の矢印と同じ流儀。
  バンド内は予約文脈なので原子と衝突しない — コンパクト形の1文字
  アクセントは従来どおり ⇀)、overline は `_` 充填(¯ はセル上部に
...
- **両極限が空の BigOp** は、基底が裸に戻れる単独原子(∑ 系記号・
  lim 系関数)なら正規形に存在しない(base が行に展開され、裸の ∑ は
  ただの1文字)。`┄ess┄sup┄` のような \op* 名前バンド(Text piece や
  複数 piece)は極限なしでも band のまま(裸に展開すると \mathrm と
  区別できず、band に戻す UI 経路もないため)。
  裸の `∑` はただの原子。
