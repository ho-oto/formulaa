...
### Cancel(打ち消し線)
- 打ち消しは**記号原子1個**を包む `Cancel(char)`(struck な Sym)。
  グリフのセルに結合ロングソリダス U+0338 を後置(`y̸`)。結合文字は
  セルを消費しないので列モデルを壊さない(パース時に前のセルのフラグへ
  折り込む。U+0336 も受理)。\cancel は LaTeX 標準ではないので
  部分サポートで割り切る: 立体ラン(Func/Roman)・テキスト・構造には
  線を引けない。
- パース: 記号原子を読んだセルのフラグを見て `Cancel` に包む。それ以外の
  セル(立体ラン・引用トークン・構造グリフ)に乗ったフラグはどこにも
  回収されず、パース終端の検査でエラー(黙って落とさない)。
- 構造への打ち消しの押し下げはエディタと LaTeX リーダーが `cancel_all` で
  行う(中の記号原子だけが struck になる)。**空の Sup/Sub
