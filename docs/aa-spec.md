...
| `^ ˜ ¯ ⇀ ˙ ¨ ˇ ˚` | 上アクセントマーク(原子の `~` `→` とは別コードポイント)。すべて**基底に密着する形で描画**: bar `¯`→`_`、hat `^`→`˰` U+02F0、tilde `˜`→`˷` U+02F7、check `ˇ`→`˯` U+02EF、ring `˚`→`˳` U+02F3、dot `˙`→`․` U+2024、ddot `¨`→`․․`(**右に1列はみ出す** — はみ出し列の基線は空白で、他に何も置けない)。浮いた元グリフも寛容に読む(breve `˘` は廃止) |
...
- すべてのマークは wide 形と同じく基底に密着して描く:
  ...
  (原子の `.` とは別字)、下バー(AST `‗`)は `¯`、under tilde
