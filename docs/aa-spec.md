...
- パース: トークンを読んだ場所でスパンのフラグを検査 — 全セル打ち消しなら
  `Cancel` に包み、一部だけならエラー。構造グリフ(罫線・括弧・格子縁・
  スクリプト)上のフラグはどのトークンにも回収されず、パース終端の検査で
  エラー(黙って落とさない)。
- 正規化: 構造を包む迷子の `Cancel` は中の各トークンへ押し下げる
  (絵として同じもの)。二重の Cancel は1枚に。**空の Sup/Sub
...
- 構造を包む Cancel は中のトークンへ押し下げ、二重の Cancel は1枚に。
